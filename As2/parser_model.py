...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
                 hidden_size=256, n_classes=3, dropout_prob=0.4):
        ...
        W = torch.empty(n_features*self.embed_size, hidden_size)
        ...
        W1 = torch.empty(hidden_size, hidden_size)
        nn.init.xavier_uniform_(W1)
        self.embed_to_hidden_weight1 = nn.Parameter(W1)

        b3 = torch.empty(hidden_size)
        nn.init.uniform_(b3)
        self.embed_to_hidden_bias3 = nn.Parameter(b3)

        self.dropout = nn.Dropout(self.dropout_prob)

        U = torch.empty(hidden_size, n_classes)
        nn.init.xavier_uniform_(U)
        self.hidden_to_logits_weight = nn.Parameter(U)

        b2 = torch.empty(n_classes)
        nn.init.uniform_(b2)
    ...
    def forward(self, w):
        ...
        W = self.embed_to_hidden_weight
        b1 = self.embed_to_hidden_bias
        U = self.hidden_to_logits_weight
        b2 = self.hidden_to_logits_bias

        W1 = self.embed_to_hidden_weight1
        b3 = self.embed_to_hidden_bias3
